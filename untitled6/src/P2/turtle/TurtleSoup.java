...
import static org.primefaces.component.signature.Signature.PropertyKeys.color;
...
public class TurtleSoup {
    ...
    public static void drawSquare(Turtle turtle, int sideLength) {
        for (int i = 0; i< 4; i++) {turtle. forward(sideLength); turtle.turn(90);
     }
    ...
    public static int calculatePolygonSidesFromAngle(double angle) {
        int sidesnum = (int)Math.rint((180/(360-angle)));
        return  sidesnum;
    }
    ...
    public static void drawRegularPolygon(Turtle turtle, int sides, int sideLength) {
        ...
        for (int i = 0; i< sides; i++){
            turtle. forward(sideLength); turtle.turn(angle);}
    }
    ...
    public static List<Double> calculateBearings(List<Integer> xCoords, List<Integer> yCoords) {
        ...
        int curX = xCoords.get(0), curY = yCoords.get(0), targetX, targetY;
        int length = xCoords.size();
        List<Double> ans = new ArrayList<>();
        for (int i = 1; i < length; i++) {
            targetX = xCoords.get(i);targetY = yCoords.get(i);
            ans.add(calculateBearingToPoint(currentBearing, curX, curY, targetX, targetY));
    ...
    public static void drawPersonalArt(Turtle turtle) {
        for(int i = 0; i<120; i++){
            drawRegularPolygon(turtle,7,2*i); turtle.turn(10);
            if(i%8==0) turtle.color(PenColor.RED);
            if(i%8==1) turtle.color(PenColor.BLUE);
            if(i%8==2) turtle.color(PenColor.CYAN);
            if(i%8==3) turtle.color(PenColor.GRAY);
            if(i%8==4) turtle.color(PenColor.GREEN);
            if(i%8==5) turtle.color(PenColor.MAGENTA);
            if(i%8==6) turtle.color(PenColor.ORANGE);
            if(i%8==7) turtle.color(PenColor.PINK);
        }
    }
